...
export default function PortfolioAssistant() {
  ...
  return <div className="fixed bottom-5 right-5 z-[60] max-w-[calc(100vw-2.5rem)] sm:bottom-6 sm:right-6">
    <AnimatePresence>
      {open && <motion.section initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 20 }} transition={{ duration: 0.2 }} className="mb-3 flex h-[min(600px,calc(100vh-8rem))] w-[min(380px,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-2xl border border-purple-400/30 bg-black/85 shadow-2xl shadow-purple-950/50 backdrop-blur-xl" aria-label="Kayden AI Assistant">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-3" aria-live="polite">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`max-w-[88%] rounded-2xl px-3 py-2 text-sm leading-6 ${message.role === "user" ? "ml-auto bg-purple-600/80" : "border border-white/10 bg-white/10 text-gray-200"}`}>{message.content}</div>)}{loading && <div className="flex gap-1 px-3 py-2 text-purple-300" aria-label="Loading"><span className="animate-bounce">•</span><span className="animate-bounce [animation-delay:120ms]">•</span><span className="animate-bounce [animation-delay:240ms]">•</span></div>}<div ref={endRef} /></div>
